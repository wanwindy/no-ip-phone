...
The server now fails fast at startup when `NODE_ENV=production` but the Redis baseline above is not met. The goal is to stop a release from silently running with in-memory rate limiting.
...
- In production-like env, set `SMOKE_AUTH_CODE` explicitly. If `SMS_PROVIDER=noop`, read the code from the server log before running the script.
